...
# Supports: Microdot, Response, send_file, app.run_task
...
class Response:
    ...
    def to_bytes(self):
        body = self.body if isinstance(self.body, (bytes, bytearray)) else self.body.encode('utf-8')
        status_line = 'HTTP/1.1 {} {}\r\n'.format(self.status, 'OK' if self.status==200 else 'ERR')
        hdrs = status_line
    ...
    async def _handle_client(self, reader, writer):
        try:
            ...
            if isinstance(resp, Response):
                ...
                await writer.awrite(Response(str(resp)).to_bytes())
